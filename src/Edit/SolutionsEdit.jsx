...
const SolutionsEdit = () => {
    ...
    const whychoose = async () => {
        try {
            let res = await fetch(`http://localhost:2233/${id}/heading/${id2}`)
            let solutionsdataa = await res.json()
            console.log({ solutionsdataa })
          
            setTitle(solutionsdataa.title)
            // setHeading2(solutionsdataa.heading2)
            setDesc(solutionsdataa.desc)
    ...
    const handleSubmit = async (e) => {
        ...
        const response = await fetch(`http://localhost:2233/solutionsMain/${id2}`, {
            method: 'PATCH',
            headers: {
